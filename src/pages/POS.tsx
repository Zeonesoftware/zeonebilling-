...
export default function POS() {
  ...
  return (
    <div className="h-full flex gap-6 overflow-hidden min-h-[600px]">
      ...
      <div className="flex-1 flex flex-col gap-4 min-w-0 h-full">
        <div className="flex items-center gap-4 shrink-0">
          <div className="relative flex-1">
            ...
            <Input 
              ref={barcodeRef}
              placeholder="F2 to search / Scan barcode..." 
              className="pl-10 h-14 shadow-sm border-slate-200 focus-visible:ring-[#237227] text-lg rounded-xl"
              value={searchTerm}
        ...
        <ScrollArea className="flex-1 min-h-0 bg-white/50 rounded-2xl border border-slate-100 p-1">
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 xxl:grid-cols-4 gap-3 p-1">
                ...
                >
                  <CardContent className="p-4 flex flex-col h-full gap-2">
                    <div className="flex justify-between items-start">
                      <div className={cn(
                        "w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 group-hover:bg-[#237227] group-hover:text-white transition-colors",
                        isLowStock && "bg-red-50 text-red-400"
                      )}>
                        {isLowStock ? <XCircle className="w-5 h-5 text-red-500" /> : <Plus className="w-5 h-5" />}
                      </div>
                    ...
                    <div className="space-y-1 mt-1 flex-1">
                      <h3 className="font-black text-sm text-slate-800 leading-tight line-clamp-2">{item.name}</h3>
                      <p className="text-[9px] uppercase font-mono text-slate-400 tracking-widest">{item.hsn ? `HSN: ${item.hsn}` : 'NO HSN'}</p>
                    </div>
                    <div className="pt-2 flex items-center justify-between border-t border-slate-50">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Selling Price</span>
                        <span className="font-black text-lg text-slate-900">₹{item.price}</span>
                      </div>
                      <Badge variant="outline" className="text-[10px] font-bold bg-emerald-50 text-emerald-700 border-emerald-100">{item.gstRate}% GST</Badge>
                    </div>
                  </CardContent>
                </Card>
              );
      ...
      {/* Cart / Checkout Side */}
      <div className="w-[420px] bg-white border border-slate-200 rounded-2xl flex flex-col shadow-2xl shadow-slate-100 overflow-hidden h-full">
        <div className="p-6 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#237227] rounded-xl flex items-center justify-center text-white shadow-lg shadow-[#237227]/20">
              <ShoppingCart className="w-5 h-5" />
            </div>
            <div>
              <h2 className="font-black text-lg text-slate-800 leading-none">Checkout</h2>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Order Session #0023</p>
            </div>
          </div>
          <Badge className="bg-black text-white rounded-full px-3 py-1 font-black text-xs">{cart.length} ITEMS</Badge>
        </div>

        <div className="p-4 bg-white border-b border-slate-50 shrink-0">
          <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 transition-all focus-within:border-[#237227] focus-within:bg-white">
            <User className="w-4 h-4 text-slate-400" />
            <div className="flex-1">
              <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Client Selection</p>
              <select 
                className="bg-transparent text-sm font-bold w-full focus:outline-none text-slate-800 cursor-pointer"
        ...
        <ScrollArea className="flex-1 min-h-0">
          <div className="p-4 space-y-3">
            {cart.map(item => (
              <div 
                key={item.itemId} 
                className="group p-3 bg-white border border-slate-100 rounded-xl hover:border-slate-200 hover:shadow-sm transition-all flex items-center gap-3"
              >
                <div className="w-12 h-12 bg-slate-50 rounded-lg flex items-center justify-center text-slate-300 font-bold text-xs uppercase overflow-hidden shrink-0">
                  {item.name.charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-black text-xs text-slate-800 uppercase tracking-tight truncate mb-0.5">{item.name}</div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-slate-400">₹{item.price} + {item.gstRate}%</span>
                    <Badge variant="outline" className="text-[8px] py-0 border-slate-100 text-slate-400 font-bold">HSN: {item.hsn}</Badge>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-end gap-2">
                  <div className="flex items-center gap-1 bg-slate-100 p-0.5 rounded-lg border border-slate-200 shrink-0">
                    ...
                    <Button 
                      variant="ghost" 
                      size="icon" 
                      className="h-6 w-6 rounded-md hover:bg-white hover:text-[#237227]"
                      onClick={() => updateQuantity(item.itemId, 1)}
                    >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-black text-sm text-slate-900">₹{item.total.toFixed(0)}</span>
                    <button 
            ...
            {cart.length === 0 && (
              <div className="flex flex-col items-center justify-center py-10 text-slate-200 border-2 border-dashed border-slate-100 rounded-3xl">
                <ShoppingCart className="w-12 h-12 mb-3 opacity-10" />
                <p className="text-xs font-bold uppercase tracking-widest">Cart is empty</p>
                <p className="text-[10px] text-slate-400 mt-1">Add items to begin</p>
              </div>
            )}
          </div>
        </ScrollArea>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 space-y-3 shrink-0">
          <div className="space-y-1.5 p-3 bg-white rounded-xl border border-slate-200 shadow-sm shrink-0">
            ...
            <div className="pt-2 mt-1 border-t border-slate-100 flex justify-between items-center">
              <span className="font-black text-[11px] text-slate-800 uppercase tracking-tighter">Payable Amount</span>
              <span className="font-black text-xl text-[#237227] tracking-tighter">₹{total.toFixed(0)}</span>
            </div>
          </div>

          <div className="grid grid-cols-5 gap-2 shrink-0">
            <Button 
              className="col-span-4 h-12 bg-[#237227] text-white hover:bg-[#1B561E] rounded-xl text-base font-black gap-2 shadow-lg shadow-[#237227]/20 border-b-4 border-[#1B561E] active:border-0 active:translate-y-1 transition-all"
            ...
            <Button 
              variant="outline" 
              className="h-12 rounded-xl border-2 border-slate-200 hover:bg-white hover:border-[#237227] hover:text-[#237227] transition-all flex flex-col items-center justify-center gap-0.5 group"
              onClick={handlePrint}
            >
              <Printer className="w-4 h-4" />
              <span className="text-[8px] font-black uppercase tracking-tighter">F10</span>
            </Button>
          </div>
